    ...
    execute: async (_client: Client, interaction: ChatInputCommandInteraction) => {
        ...
        try {
            ...
            if (user) {
                ...
                const updateClassBtn = new ButtonBuilder()
                    .setCustomId(`schedule_${interaction.user.id}_start_${ctxId}`)
                    .setLabel("Update Class Information")
                    .setStyle(ButtonStyle.Primary)
                    .setEmoji('📚');
            ...
            console.error(`[${tags.Error}] Failed to load schedule setup for user [UID: ${interaction.user.id}]`);
            console.error(e);
            ...
            const errorContainer = new ContainerBuilder()
                ...
                .addSeparatorComponents(s => s)
                .addTextDisplayComponents(t => t.setContent("An error occurred while loading your schedule setup. Please try again later."));
